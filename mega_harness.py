...
# ---------- parallel (variant, block) evaluation over all candidates ----------
import pickle
from multiprocessing import Pool
from scipy.optimize import minimize
HOLD = set(series_order[-45:])
BAN_F = ["cap","thr","revu","revw","map","den","selfban"]
PROT_F = ["cap","ls","ownw","map","selfprot"]
...
def fit_kind(kind, names, train, rho):
...
def predict_p(d, names, w, logT, sidx, rho):
    T = math.exp(logT[sidx[d["slot"]]])
    u = habit(d, rho).copy()
    for j, nm in enumerate(names):
        u = u + w[j] * (d["feats"][nm] if nm in d["feats"] else d["_x"][nm])
    u = u / T
    m = u.max(); p = np.exp(u - m); return p / p.sum()

def lookahead_probs(d, names, w, logT, sidx):
    lk = d["look"]; surv = np.ones(NH)
    for osl in lk["slots"]:
        u = lk["alpha"][osl].copy()
        for j, nm in enumerate(names):
            if nm in ("den", "urg", "haz", "hazls"): continue
...
def eval_task(argt):
    vname, rho, use_haz, use_urg, bi = argt
    blk = set(BLOCKS[bi])
    ...
    pw, plT, psx = fit_kind("protect", PROT_F, ptr, rho)
    bnames = list(BAN_F)
    if use_urg:
        bnames.append("urg")
        for d in btr + bte:
            d["_x"]["urg"] = (lookahead_probs(d, PROT_F, pw, plT, psx)[d["legal"]]
                              if d["look"] else np.zeros(len(d["legal"])))
    bw, blT, bsx = fit_kind("ban", bnames, btr, rho)
    pnames = list(PROT_F)
    if use_haz:
        pnames += ["haz", "hazls"]
        for d in ptr + pte:
            hz = (lookahead_probs(d, bnames, bw, blT, bsx)[d["legal"]]
                  if d["look"] else np.zeros(len(d["legal"])))
            d["_x"]["haz"] = hz; d["_x"]["hazls"] = hz * d["feats"]["ls"]
        pw, plT, psx = fit_kind("protect", pnames, ptr, rho)
    out = []
    for d, names_, w_, lT_, sx_ in         [(x, bnames, bw, blT, bsx) for x in bte] + [(x, pnames, pw, plT, psx) for x in pte]:
        p = predict_p(d, names_, w_, lT_, sx_, rho)
        rank = int((p > p[d["y"]]).sum()) + 1
        out.append((d["kind"], -math.log(max(p[d["y"]], 1e-12)), rank))
    return (vname, bi, out)

VARIANTS = [
    ("A ref V5+T",            None, False, False),
    ("B mix rho=0.90",        0.90, False, False),
    ("C mix rho=0.97",        0.97, False, False),
    ("D mix rho=0.99",        0.99, False, False),
    ("E hazard",              None, True,  False),
    ("F hazard+urgency",      None, True,  True),
    ("G mix.97+haz+urg",      0.97, True,  True),
]
tasks = [(v[0], v[1], v[2], v[3], bi) for v in VARIANTS for bi in range(len(BLOCKS))]
print(f"{len(tasks)} tasks over {os.cpu_count()} cpus", flush=True)
import os
with Pool(min(40, os.cpu_count() - 2)) as pool:
    results = pool.map(eval_task, tasks)
agg = defaultdict(lambda: {"ban": [0.0,0,0,0], "protect": [0.0,0,0,0]})
for vname, bi, rows in results:
    for kind, ll, rank in rows:
        a = agg[vname][kind]
        a[0] += ll; a[1] += 1; a[2] += (rank == 1); a[3] += (rank <= 3)
print()
for vname, _, _, _ in VARIANTS:
    line = vname.ljust(22)
    for kind in ("ban", "protect"):
        nl, n, t1, t3 = agg[vname][kind]
        line += f" | {kind}: ll {nl/n:.4f} top1 {100*t1/n:.1f}% top3 {100*t3/n:.1f}%"
    print(line, flush=True)
print("MEGA_DONE", flush=True)
