...
for ri, r in enumerate(recs):
    ...
    for ph in sorted(by_phase):
        ...
        for a in acts:
            ...
            if kind == "ban":
                _tot = kind_n["ban"] + 0.25 * NH
                _pa = (kind_c["ban"] + 0.25) / _tot
...
def look_probs(d, names, w, logT, sidx, pl, kalpha_by_slot):
    # exact state recursion: H(h) = p1(h) + sum_{j!=h} p1(j) p2(h | j removed)
    lk = d["look"]
    def evec(osl):
        u = lk["alpha"][osl].copy()
        for j, nm in enumerate(names):
            if nm in ("den", "plh"): continue
        ...
        return np.exp(u - m)
    slots = lk["slots"]
    e1 = evec(slots[0]); p1 = e1 / e1.sum()
    if len(slots) == 1: return p1
    e2 = evec(slots[1]); Z2 = e2.sum()
    den = np.maximum(Z2 - e2, 1e-12)
    Ssum = float((p1 / den).sum())
    return np.clip(p1 + e2 * (Ssum - p1 / den), 0.0, 1.0)
...
def set_ban_alpha(kappa, expo):
    for d in decisions:
        if d["kind"] != "ban": continue
        den = (d["hav"] if expo else d["hn"]) + kappa
        d["alpha"] = np.log((d["hc"] + kappa * d["hpa"]) / den)

set_ban_alpha(4.0, True)
run(False, "EXPOSURE k4 EXACT-HAZ")
print("HAZEX_DONE", flush=True)
