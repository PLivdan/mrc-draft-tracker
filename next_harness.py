...
team_hist = defaultdict(list)   # per team: (lineup frozenset, ban vec, prot vec) per map
...
for ri, r in enumerate(recs):
    ...
    def _roster_hab(side):
        today = frozenset(p["player_id"] for p in r["lineups"][side])
        nb = np.zeros(NH); npv = np.zeros(NH); wsum = 0.0; wt = 1.0
        for lu, bv, pv in reversed(team_hist[tn[side]]):
            w = wt * (len(today & lu) / 6.0)
            nb += w * bv; npv += w * pv; wsum += w
            wt *= DELTA
        return nb / (wsum + 1.0), npv / (wsum + 1.0)
    selfb_r, selfp_r = {}, {}
    for s_ in ("blue", "red"): selfb_r[s_], selfp_r[s_] = _roster_hab(s_)
    ...
    for side in ("blue", "red"):
        ...
        team_hist[team].append((frozenset(p["player_id"] for p in r["lineups"][side]),
                                pend_ban[team].copy(), pend_prot[team].copy()))
...
def run(pl, label, blend=False):
    ...
    for bi, blk_ in enumerate(BLOCKS):
        ...
        for d, names_, w_, lT_, sx_ in             [(x, namesB, bw, blT, bsx) for x in bte] + [(x, PROT_F, pw, plT, psx) for x in pte]:
            ...
            u0 = d["alpha"] - d["alpha"].max()
            p0 = np.exp(u0); p0 = p0 / p0.sum()
            sk = skill[d["kind"]]
            sk[0] += (-math.log(max(p0[d["y"]], 1e-12))) - (-math.log(max(p[d["y"]], 1e-12)))
            sk[1] += 1
            if d["kind"] == "ban":
    ...
    print("   skill over league baseline (nats/decision): " +
          " ".join(f"{k} {v[0]/max(v[1],1):+.4f}" for k, v in skill.items()), flush=True)
...
set_alpha(4.0, True, True)   # frozen v3 habits everywhere
if MODE == "main":
    run(False, "REF v3")
